...
type AccountPageProps = {
  searchParams: Promise<{ denied?: string; "email-change"?: string }>;
};

export default async function AccountPage({ searchParams }: AccountPageProps) {
  const user = await requireCurrentUser();
  const { denied, "email-change": emailChangeToken } = await searchParams;
  ...
  return (
    ...
    >
      <AccountClient currentUser={user} deniedPage={denied} emailChangeToken={emailChangeToken} pageAccess={pageAccess} />
    </AppShell>
  );
}